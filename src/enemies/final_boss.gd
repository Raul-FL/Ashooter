...
func set_damage(amount):
	...
	if hp <= 0:
		for i in get_tree().get_nodes_in_group("stage"):
			i.queue_free()
		get_tree().get_nodes_in_group("final").front().show()
		get_tree().get_nodes_in_group("final").front().get_node("AnimationPlayer").play("dd")
#		get_tree().get_nodes_in_group("player").front().set_process(false)
#		get_tree().get_nodes_in_group("player").front().monitoring = false
#		get_tree().get_nodes_in_group("HUD").front().queue_free()
...
func _on_monster_tree_exiting():
#	get_tree().get_nodes_in_group("player").front().set_process(false)
	get_tree().get_nodes_in_group("player").front().monitoring = false
	get_tree().get_nodes_in_group("player").front().get_node("hurt_timer").stop()
	get_tree().get_nodes_in_group("player").front().set_process(false)
	get_tree().get_nodes_in_group("player").front().get_node("spaceship_day").show()
	get_tree().get_nodes_in_group("player").front().get_node("spaceship_night").hide()
	get_tree().get_nodes_in_group("player").front().get_node("AnimationPlayer").play("finished_stage")
	if stage_clear:
		get_parent().get_node("Label").show()
